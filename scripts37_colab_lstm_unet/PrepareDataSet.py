...
def train_LSTM_UNET():
    ...
    # متعادل‌سازی داده‌ها با SMOTE (بدون تغییر در بخش آماده‌سازی داده‌ها، فقط روی داده‌های بارگذاری‌شده اعمال می‌شود)
    smote = SMOTE(random_state=42)
    X_resampled, Y_resampled = smote.fit_resample(X.reshape(-1, X.shape[1] * X.shape[2]), Y)
    X_resampled = X_resampled.reshape(-1, X.shape[1], X.shape[2])
    print(f"Shape of X_resampled: {X_resampled.shape}")
    print(f"Shape of Y_resampled: {Y_resampled.shape}")
    print("Distribution in Y_resampled:", np.unique(Y_resampled, return_counts=True))
    ...
    # تبدیل داده‌ها به فرمت دوبعدی برای U-Net
    X_train_full_2d = X_train_full[..., np.newaxis]  # (samples, sequence_length, vector_length, 1)
    X_test_2d = X_test[..., np.newaxis]  # (samples, sequence_length, vector_length, 1)

    # تعریف ورودی‌ها
    inputs_unet = Input(shape=(X.shape[1], X.shape[2], 1))  # ورودی U-Net
    inputs_lstm = Input(shape=(X.shape[1], X.shape[2]))     # ورودی LSTM

    # شاخه U-Net با Conv2D
    # Encoder
    conv1 = Conv2D(64, (3, 3), padding='same')(inputs_unet)
    ...
    crop_conv3 = Cropping2D(cropping=((0, 0), (0, 1)))(conv3)  # تنظیم ابعاد
    concat5 = Concatenate()([up5, crop_conv3])
    ...
    crop_conv2 = Cropping2D(cropping=((1, 0), (1, 0)))(conv2)  # تنظیم ابعاد
    concat6 = Concatenate()([up6, crop_conv2])
    ...
    crop_conv1 = Cropping2D(cropping=((1, 1), (2, 2)))(conv1)  # تنظیم ابعاد
    concat7 = Concatenate()([up7, crop_conv1])
    ...
    # تبدیل خروجی U-Net به بردار
    unet_output = GlobalAveragePooling2D()(conv7)

    # شاخه LSTM (بدون تغییر)
    lstm1 = Bidirectional(LSTM(128, return_sequences=True))(inputs_lstm)
    ...
    # ساخت مدل
    model = Model(inputs=[inputs_unet, inputs_lstm], outputs=output)
    ...
    Y_pred_prob = model.predict([X_test_2d, X_test])
    ...
    cm_path = os.path.join(docs_dir, 'confusion_matrix_improved.png')
    plt.savefig(cm_path)
    plt.show()
    ...
    # ذخیره مدل
    model.save('final_LSTM_UNET_model_improved.h5')
