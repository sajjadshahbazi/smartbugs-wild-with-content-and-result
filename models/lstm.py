...
class LSTM_Model:
    def __init__(self, data, name="", batch_size=args.batch_size, lr=args.lr, epochs=args.epochs, dropout=args.dropout,
        ...
        labels  = np.stack(data.iloc[:, 1].values)
        # vectors = vectors.reshape()
        vectors = data.iloc[:, 0].values
        # print("labels : ", labels)
        ...
        print(f"Positive indices: {positive_idxs}")
        print(f"Negative indices: {negative_idxs}")

        undersampled_negative_idxs = np.random.choice(negative_idxs, len(positive_idxs), replace=False)
        resampled_idxs = np.concatenate([positive_idxs, undersampled_negative_idxs])
        # print(f"vectores : {vectors[resampled_idxs]}, labels : {labels[resampled_idxs]}")
